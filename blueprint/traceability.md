...
| R09 | [SRC] | Buy/Sell và Entry/Exit visualization | `specs/visualization.md`, `specs/chart-overlay.md` | 10, 22, 24 | Yes | Partial | Partial | Canonical chart-overlay/marker unit tests and experiment overlay API exist; entry/exit marker mang `sequence_no`, trade row highlight marker cùng sequence, trade API cursor ổn định và CSV export streams provenance. Còn thiếu browser click-to-highlight acceptance. |
...
| R14 | [SRC] | Modifiability, scalability, realtime, reliability, performance, maintainability, observability, reproducibility | `design.md` §8–§11, `specs/observability.md` | 07–09, 14–20, 24, 35–39 | Yes | Partial | Partial | Internal health/ready/Prometheus metrics include request, queue/outbox, search, sentiment, agent-run and sandbox gauges; full crash-recovery/load benchmark/dashboard evidence remains. |
...
| A02 | [SRC-ADD] | Natural language hoặc URL → draft cho user review | `specs/strategy-authoring.md`, `specs/agent-architecture.md` | 21, 25–27, 33 | Yes | Partial | Partial | `tests/test_authoring.py`, `tests/test_api.py` và integration queue: public command → persisted `DRAFT_CREATED` + leased agent job → immutable review/approve → runtime. URL chỉ đi qua origin allowlist, DNS/public-IP check tại từng redirect, pinned fetch và sanitize trước Designer; test chặn redirect private-network/prompt injection. |
| A03 | [SRC-ADD] | Sinh Python strategy file và bounded repair loop | `specs/strategy-authoring.md`, `specs/agent-architecture.md` | 21, 26, 28, 29, 33 | Yes | Partial | Partial | Deterministic DSL artifact, AST/preflight, Docker sandbox no-network/read-only/non-root, durable retry/cancel/heartbeat, per-state CAS resume và hash/mã lỗi từng Designer attempt đã có unit/integration evidence. Custom Python proposal chỉ repair khi policy đã pass nhưng sandbox fail: `StrategyRepairAgent` tạo source replacement tối đa 3 lần, policy/sandbox chạy lại; import/API policy violation không đi tới model. Review/deploy vẫn bắt buộc. |
...
| A05 | [SRC-ADD] | Trade row đủ pair/time/LONG-SHORT/USD notional/prices/SL-TP/cost/spread-slippage/profit | `specs/backtest.md`, `specs/visualization.md` | 06, 23, 24 | Yes | Partial | Partial | API/schema/persistence có pair, quote currency, entry/exit notional, fee, spread, slippage, gross/net; engine/integration chứng minh `net = gross - costs`; CSV có provenance và stream qua public API; overlay API có entry/SL/TP/exit marker. Còn thiếu browser acceptance cho toàn bộ cột/tooltip. |
...
| A09 | [SRC-ADD] | MA, Bollinger, SMC và strategy bổ sung | `specs/strategy-registry.md`, `specs/strategy-authoring.md` | 03, 10, 21, 22, 36 | Yes | Partial | Partial | MA/BB/SMC đều đăng ký qua plugin registry; SMC BOS causal fixture và catalog API test có evidence. Còn thiếu browser/backtest demo bao phủ toàn bộ catalog. |
...
| P04 | [PD] | Six logical agents, deterministic orchestrator, typed least-privilege tools | `specs/agent-architecture.md` | 25–33 | Yes | Partial | Partial | `ToolRegistry` có ma trận allowlist cố định cho 6 role; Strategy Authoring có persisted job, lease heartbeat/reclaim, bounded retry/cancel, per-state CAS resume, audit và typed tool-invocation hashes. Còn thiếu orchestrator runtime chung cho News/Discovery/Insight và audit cho tool bị deny/fail. |
| P05 | [PD] | DSL-backed authoring MVP; custom Python requires review/build/deploy | `specs/strategy-authoring.md` | 21, 26, 28, 29, 33 | Yes | Partial | Partial | DSL compile/review/hash/publish được integration test; custom Python source proposal qua AST policy + Docker isolated compile, approval phát `StrategyCustomArtifactApprovedForDeployment` và không hot-load/không vào Registry. Sandbox-only failure được `StrategyRepairAgent` sửa tối đa ba artifact khác nhau rồi policy/sandbox lại. PR/build/deploy executor còn là ranh giới triển khai bên ngoài. |
