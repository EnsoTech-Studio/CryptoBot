"""MACD strategy plugin used in the architecture-demo scenario.

Demo integration target:
    app/domain/strategy/plugins/macd.py

The plugin only implements the shared Strategy contract.  It does not import
the database, HTTP API, worker, evaluator, or frontend.
"""
...
class MACDStrategy:
    """Emit BUY/SELL only when the MACD line crosses its signal line."""
    ...
    def analyze(self, context: AnalysisContext) -> Signal:
        ...
        confidence = min(1.0, abs(line - signal) / price) if price else 0.0
        if line > signal and previous_line <= previous_signal:
            return Signal(action=ACTION_BUY, price=price, confidence=confidence)
        if line < signal and previous_line >= previous_signal:
            return Signal(action=ACTION_SELL, price=price, confidence=confidence)
        return Signal(action=ACTION_HOLD)
